...
##    Fan speeds are scaled PWM (0 - 255) or RepRap (0.0 - 1.0) depending on Cura's "Scale Fan Speed 0-1" {machine_scale_fan_speed_zero_to_one}.
...
class AddCoolingProfile_GV(Script):

    def getSettingDataString(self):
        return """{
            "name": "Advanced Cooling Fan Control GV",
            "key": "AddCoolingProfile_GV",
            "metadata": {},
            "version": 2,
            "settings":
            {
    ...
    def initialize(self) -> None:
        super().initialize()
        try:
            if Application.getInstance().getGlobalContainerStack().getMetaDataEntry("post_processing_scripts") != None:
