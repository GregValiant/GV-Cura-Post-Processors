...
class TimedCoolDown_GV(Script):
    ...
    def getSettingDataString(self):
        return """{
            "name": "Timed Cool Down Period GV",
            "key": "TimedCoolDown_GV",
            "metadata": {},
            "version": 2,
            "settings":
            {
                ...
                "wait_time":
                {
                    ...
                    "description": "Hold the bed temp at the 'Bed Start Out Temperature' for this amount of time (in decimal hours).  When the time expires the timed cool down will start.",
                    "type": "float",
                    "default_value": 0,
                    "unit": "Decimal Hrs "
                },
                "startout_temp":
                {
                    ...
                    "description": "Enter the temperature to start at.  This is typically the bed temperature during the print but can be something else.",
                    "type": "int",
                    "value": 30,
                    "unit": "Degrees ",
                    "minimum_value": 30,
                ...
                {
                    "label": "Move to MaxZ",
                    "description": "Enable this setting to move the nozzle to 'Machine_Height - 10' to allow the print to be covered.",
                    "type": "bool",
                    "default_value": false,
                    "enabled": true
                },
    ...
    def execute(self, data):
        ...
        bed_temperature = self.getSettingValueByKey("startout_temp")
        ...
        max_z = str(int(Application.getInstance().getGlobalContainerStack().getProperty("machine_height", "value")) - 10)
        ...
        # Step the bed/chamber temps down and add each step to the anneal string--------------------------
        ...
        for num in range(len(lines)-1,-1,-1):
            if lines[num].startswith("M140 S0"):
                lines[num] = ";M140 S0 ;Shutoff Overide - Timed Cool Down"
                data[len(data)-1] = "\n".join(lines)
        ...
        if anneal_type == "bed_chamber" and heated_chamber:
            for num in range(0,len(lines)-1,1):
                if lines[num].startswith("M141 S0"):
                    lines[num] = ";M141 S0 ;Shutoff Overide - Timed Cool Down"
                    data[len(data)-1] = "\n".join(lines)
        ...
        #The Anneal string is the new end of the gcode so move the 'Wnd of Gcode' comment line in case there are other posts running
