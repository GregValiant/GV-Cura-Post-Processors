...
#    Added options:
#      'Start Layer and End layer'
#     'Initial Replacement Only'
#      'Ignore StartUp G-code' and 'Ignore Ending G-code'

import re
from ..Script import Script

class SearchAndReplace_GV(Script):
    """Performs a search-and-replace on the g-code.
    """

    def getSettingDataString(self):
        return """{
            "name": "Search and Replace GV",
            "key": "SearchAndReplace_GV",
            "metadata": {},
            "version": 2,
            "settings":
            {
                ...
                "search_end":
                {
                    ...
                    "description": "Use the Cura Preview layer numbering.  Enter '-1' to replace to the end of the file.  Enter any other layer number and the replacements will conclude at the end of that layer.",
                    "type": "int",
                    "default_value": -1,
                    "minimum_value": -1,
                    "enabled": "enable_layer_search"
            ...
            for num in range(start_index, end_index,1):
                ...
                for index, line in enumerate(lines):
                    if re.match(search_regex, line) and replaceone == False:
                        ...
                        break
                if replaceone: break
            ## If there wasn't a replacement then maybe the search string is multi-line
            if not replaceone:
                for num in range(start_index, end_index,1):
                    if re.match(search_regex, data[num]) and replaceone == False:
                        data[num] = re.sub(search_regex, replace_string, data[num])
                        replaceone = True
                        break
